...
public class Library implements ActionListener{
    ...
    JButton b1,b2,b3,b4;
	public Library() {
	 f = new JFrame("'My Library Applicatin'");//heading mate
	 f.setVisible(true);
	 f.setSize(400,440);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==b1){
		  try {
			  ...
			  Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/java_4","root","");//connection establish.
			  String sql ="insert into library(bname,aname,quantity,price) values('"+t2.getText()+"','"+t3.getText()+"','"+t4.getText()+"','"+t5.getText()+"')";
			...
			try {
				Class.forName("com.mysql.jdbc.Driver");
				  Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/java_4","root","");
				String sql ="update library set bname=?,aname=?,quantity=?,price=? where id=?";
				PreparedStatement pst = (PreparedStatement) conn.prepareStatement(sql);
